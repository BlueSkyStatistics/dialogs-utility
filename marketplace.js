...
// === TEMPLATE RENDERER ===
// Direct template literal rendering replaces Squirrelly template engine for better performance.
// Each render method produces HTML via native JS template literals, eliminating
// the overhead of template parsing, regex processing, and interpretation at render time.
class TemplateRenderer {
    renderChapter(id, chapterName) {
        return `<a class="nav-link" id="market_chapter_${id}" data-toggle="pill" href="#market_tab_${id}" role="tab" aria-controls="market_tab_${id}" aria-selected="false">${chapterName}</a>`;
    }

    renderTab(id, chapterName, cards) {
        return `<div class="tab-pane fade" id="market_tab_${id}" role="tabpanel" bs-tab="${chapterName}" aria-labelledby="market_chapter_${id}">
            ${cards.join('')}
        </div>`;
    }

    renderCard({dialog, chapter, child = '', userd = false, uninstall = '', install = '', hidden = false}) {
        const modalId = dialog.modal || dialog.modal_id || '';
        const pillClass = userd ? 'bg-success' : 'bg-primary';
        const pillText = userd ? 'User Dialog' : 'Base dialog';

        let buttonsHtml;
        if (userd) {
            const uninstallHidden = uninstall === 'hidden' ? ' hidden' : '';
            const installHidden = install === 'hidden' ? ' hidden' : '';
            buttonsHtml = `
                    <button type="button" class="btn btn-sm btn-outline-warning btn-refresh float-right${uninstallHidden}"
                        data-child="${child}" data-modal="${modalId}" 
                    ...
                    <button type="button" 
                        class="btn btn-sm btn-outline-danger float-right${uninstallHidden}" 
                        data-child="${child}" data-modal="${modalId}"
                        data-file="${child}" data-section="${chapter}"
                    ...
                    <button type="button" 
                        class="btn btn-sm btn-outline-primary float-right${installHidden}"
                        data-child="${child}" data-modal="${modalId}"
                        data-file="${child}" data-section="${chapter}"
                        onclick="global.marketplaceActionHandler.addDialog(event, '${child}', '${modalId}', '${chapter}')">
                        Install
                    </button>`;
        } else {
            const hideHidden = hidden ? ' hidden' : '';
            const showHidden = hidden ? '' : ' hidden';
            buttonsHtml = `
                    <button type="button" class="btn btn-sm btn-outline-secondary float-right${hideHidden}" data-child="${child}" data-modal="${modalId}" data-action="hide" onclick="handleMarketActionClick(this)">Hide</button>
                    <button type="button" class="btn btn-sm btn-outline-success float-right${showHidden}" data-child="${child}" data-modal="${modalId}" data-action="show" onclick="handleMarketActionClick(this)">Show</button>`;
        }
                    ...
                    <div class="col-8 title">
                        <div class="d-flex">
                            ...
                            <div class="d-inline-flex ml-2">
                                <div class="${pillClass} rounded-pill pl-3 pr-3" style="height: 20px;">
                                    ${pillText}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-4">${buttonsHtml}
                    </div>
                </div>
            </div>
            <div class="card-body">
    ...
    renderUserDialogCard({dialog, chapter, child, uninstall = '', install = ''}) {
        const modalId = dialog.modal || dialog.modal_id || '';
        const uninstallHidden = uninstall === 'hidden' ? ' hidden' : '';
        const installHidden = install === 'hidden' ? ' hidden' : '';
            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-4">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-danger float-right${uninstallHidden}" 
                            data-child="${child}" data-modal="${modalId}"
                            data-file="${child}" data-section="${chapter}"
                            data-action="uninstall" onclick="handleMarketActionClick(this)">Uninstall</button>
                        <button type="button" class="btn btn-sm btn-outline-primary float-right${installHidden}"
                            data-child="${child}" data-modal="${modalId}"
                            data-file="${child}" data-section="${chapter}"
                            onclick="global.marketplaceActionHandler.addDialog(event, '${child}', '${modalId}', '${chapter}')">Install</button>
                    </div>
                </div>
            </div>
            <div class="card-body">
    ...
    renderBaseDialogCard({dialog, chapter, child, hidden = false}) {
        const modalId = dialog.modal || dialog.modal_id || '';
        const hideHidden = hidden ? ' hidden' : '';
        const showHidden = hidden ? '' : ' hidden';
        ...
        return `<div class="card text-white" bs-tab="modules">
            <div class="card-header">
                <div class="row">
                    <div class="col-8 title">
                        <div class="d-flex">
                            <div class="d-inline-flex"><h6>${module.name}</h6></div>
                            <div class="d-inline-flex ml-2">
                                <div class="bg-success rounded-pill pl-3 pr-3" style="height: 20px;">${module.version}</div>
                            </div>
                            <div class="d-inline-flex ml-2">
                                <div class="bg-primary rounded-pill pl-3 pr-3" style="height: 20px;">${module.type}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-4">
    ...
    renderModal(modalId, modalLabel, chapters, tabs, modules) {
        return `<div class="modal left fade" id="${modalId}" tabindex="-1" role="dialog" 
            data-backdrop="false" data-keyboard="false"
            aria-labelledby="${modalId}Label" aria-hidden="true">
            <div class="modal-dialog modal-lg marketplace" role="document">
                <div class="modal-content" id="${modalId}modelcontentdiv">
                    <div class="modal-header pr-1 pl-3">
                        <div class="row w-100">
                            ...
                            <div class="col-5 float-right pt-2">
                                ...
                                <button type="button" action="help" class="close btn-tooltip mr-0 enable-tooltip" id="${modalId}Help"
                        ...
                        <div class="d-flex flex-nowrap">
                            ...
                            <div class="tab-content flex-fill" id="v-pills-tabContent">
                                <div class="tab-pane fade show active" id="market_tab_modules" role="tabpanel" bs-tab="modules" aria-labelledby="market_chapter_modules">
                                    ...
                                    <div class="input-group mb-3 pr-3" id="${modalId}Search">
                                ...
                                <div class="tab-pane fade" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
                                    <div class="tab-content tab-content-black" id="${modalId}_action_type_content">
                                        <div class="tab-pane fade show active p-3" id="${modalId}_action_type_upload" role="tabpanel" aria-labelledby="${modalId}_action_type_upload_tab">
                                            <div class="hidden" id="${modalId}AddMarketplace">
                                                <label for="formFile" class="form-label">Add path to Marketplace Dialogs</label>
                                                <div>
                                                    <button type="button" class="btn btn-upload" action="submit" id="${modalId}Submit">Select Folder</button>  
                                                </div>
                                            </div>
                                            <div class="hidden" id="${modalId}AddDialog">
                                                ...
                                                <div class="row">
                                                    <div class="col">
                                                        <div class="d-flex mr-3 justify-content-between">
                                                            ...
                                                            <button type="button" class="btn btn-upload btn-path" id="${modalId}DialogLocation" onclick="openDialogsFolder()"></button>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row mt-1">
                                                ...
                                                <div class="row mt-1">
                                                    <div class="col">
                                                        <button type="button" class="btn btn-upload" action="save" id="${modalId}Save">Upload</button>  
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane fade show p-3" id="${modalId}_action_type_git" role="tabpanel" aria-labelledby="${modalId}_action_type_git_tab">
                                            <div id="${modalId}GitPackage">
                                                <div class="d-flex mr-3 justify-content-between">
                                                    <div class="d-inline-flex flex-fill">
                                                        <label class="form-label m-3 small">Url</label>
                                                        <input type="text" class="form-control" placeholder="" id="${modalId}GitUrl">
                                                    </div>
                                                    <div class="d-inline-flex flex-fill">
                                                        <label class="form-label m-3 small">Branch</label>
                                                        <input type="text" class="form-control" placeholder="" id="${modalId}GitBranch" value="main">
                                                    </div>
                                                </div>
                                                <div class="d-flex mr-3 justify-content-between">
                                                    <div class="d-inline-flex flex-fill">
                                                        <label class="form-label m-3 small">User</label>
                                                        <input type="text" class="form-control" placeholder="" id="${modalId}GitUsr">
                                                    </div>
                                                    <div class="d-inline-flex flex-fill">
                                                        <label class="form-label m-3 small">Passkey</label>
                                                        <input type="text" class="form-control" placeholder="" id="${modalId}GitPwd">
                                                    </div>
                                                </div>
                                                <div class="d-flex mr-3 justify-content-between">
                                                    <div class="d-inline-flex flex-fill">
                                                        <label class="form-label m-3 small">SSHKey</label>
                                                        <input type="text" class="form-control" placeholder="" id="${modalId}GitKey">
                                                    </div>
                                                </div>
                                                <div class="d-flex mr-3">
                                                    <div class="d-inline-flex justify-content-center flex-fill">
                                                        <button type="button" class="btn btn-upload btn-path" action="submit" id="${modalId}GitSubmit">Add Package</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        switch (action) {
            case 'hide': {
                ...
                if (!hiddenObjects.includes(child)) {
                    console.log('hiding dialog:', child);
...
class Marketplace {
        ...
        try {
            ...
            if (dialog) {
                ...
                return isUserDialog ?
                    ...
                    this.templateRenderer.renderBaseDialogCard(options);
            }
        } catch (error) {
    ...
    _addTab(chapter, cards) {
        this.tabs.push(this.templateRenderer.renderTab(
            chapter.tab.replace(/[^A-Z0-9]/ig, "_"),
            chapter.name,
            cards
        ));
    ...
    _processModules() {
        ...
        installedModules.forEach(module => {
            ...
            modules.push(this.templateRenderer.renderModuleCard(module));
        });
